...
def SimFlFh_Ice(T,*ZHPdata):
    ...
    Fico = np.array([1.9066e-05,3.03e-09,-5.6556e-16,-4.3158e-18,-69954,24.119])*(1/R)  ## Constants for Ice Equation state 
    
    fhw0 = 33.5367                         ## Fugacity of water in a filled N2-hydrate at ref temperature and pressure
    Temperature = np.linspace(200,300,21)  ## Hr temperature range
    ...
    lnfwhS = Hterm + Zterm + lnfhw0  ## log(fug of water in hydrate phase at T and input pressure)

    #### Using fitted EOS for fugacity of water in ice ####
...
def SimFlFh_Water(T,*ZHPdata):
    ...
    Flco = np.array([19.074e-06,-30.365e-10,-23.990e-15,56.020e-18,-74.170e03,58.817])*(1/R); ## Constants for Liquid water Equation state 
    


    fhw0 = 33.5367          ## Fugacity of water in a real Ar-hydrate at ref temperature and pressure
    Temperature = np.linspace(270,300,7)  ### Hr temperature range
...
T0 = 220              ## Starting guess for finding T. Give a suitable initial guess, or even aan array
t0 = 280              ## Reference temperature of Hydrate

#### Specify Pressures at which dissociation temperature is to be calculated ####
#Pressure = 1e5*np.array([51.2086,60.5351,71.5602,80.6444,100,121.0724,143.123]) ## Pressure at which the eqm T is calculated -- Ice-Hydrate
Pressure = 1e6*np.array([20,30,40,50,60,70,80])                                  ## Pressure at which the eqm T is calculated -- Liquid Water-Hydrate
...
#### Integrating Z term ####
    xHgV = ETPVhyd[nz*a:nz*(a+1),2]      ## Taking the values needed to integrate
    xHwV = 1 - xHgV                      ## isotherm at T=t0, upto different
    P = ETPVhyd[nz*a:nz*(a+1),1]         ## Pressure(a).
    Vhyd = ETPVhyd[nz*a:nz*(a+1),4]
    HRgas, fug, Vgas = N2gasprop(t0,P)   ## Calculating fugacity of Nitrogen at t0 and P.
