...
def SimFlFh_Ice(T,*ZHPdata):
    ...
    fhw0 = 31.2272           ## Fugacity of water in a filled Ar-hydrate at ref temperature and pressure
    Temperature = np.linspace(200,260,13)  ### Hr temperature range
...
def SimFlFh_Water(T,*ZHPdata):
    ...
    Temperature = np.linspace(260,310,51)  ### Hr temperature range
...
T0 = 245              ## Starting guess for finding T. Give a suitable initial guess, or even an array
t0 = 280              ## Reference temperature of Hydrate

#### Specify Pressures at which dissociation temperature is to be calculated ####
Pressure = 1e6*np.array([0.8,1.6,2.4,3.2,4.0,5.0])               ## Pressure at which the eqm T is calculated -- Ice-Hydrate
#Pressure = 1e6*np.array([7.01,7.18,7.37,7.79,8.03,8.21,8.49])   ## Pressure at which the eqm T is calculated -- Liquid Water-Hydrate
...
#### Integrating Z term ####
    ...
    xHwV = 1.0 - xHgV                    ## isotherm at T=t0, upto different
