...
def SimFlFh_Ice(T,*ZHPdata):
    ...
    fhw0 = 31.2272           ### Fugacity of water in a real Ar-hydrate
    Temperature = np.linspace(200,260,13)  ### Enthalpy temperature range for Argon_Ice
...
def SimFlFh_Water(T,*ZHPdata):
    ...
    fhw0 = 31.2272           ### Fugacity of water in a real Ar-hydrate
    Temperature = np.linspace(260,310,51)  ### Enthalpy temperature range for Argon_Ice

    lnfhw0 = math.log(fhw0)
    OnebyT = 1/Temperature
    poly = np.polyfit(OnebyT,HconstP,2)    ### Fitting H integrand with 1/T as variable for N2 and Argon.
    poly = poly/R


    ###### Evaluating the integral of Hr term at instantaneous temperature T ######
    Hterm = poly[0]*(1/3)*((1/T)**3 - (1/t0)**3) + poly[1]*(1/2)*((1/T)**2 - (1/t0)**2) + poly[2]*((1/T) - (1/t0))       ### N2 and Argon
...
T0 = 245              ## Starting guess for finding T
t0 = 280              ## Reference temperature of Hydrate

Pressure = 1e6*np.array([0.8,1.6,2.4,3.2,4.0,5.0])   ## Pressure (Ar) at which the eqm T is calculated -- Ice
#Pressure = 1e6*np.array([7.01,7.18,7.37,7.79,8.03,8.21,8.49])   ## Pressure (Ar) at which the eqm T is calculated -- Water
...
#### Integrating Z term ####
    ...
    HRgas, fug, Vgas = Argasprop(t0,P)   ## For fugacity of Argon
